...
class ChessConvNet:
    def __init__(self, model_structure, load_model=False, model_path="version_64x12/model/"):
        self.model_structure = model_structure
    ...
    def train(self, data, validation=None, return_history=False):
        ...
        if torch.cuda.is_available():
            print("Using GPU")
            self.data_set = data_utils.TensorDataset(torch.from_numpy(data[:, :768]).float().cuda(), torch.from_numpy(data[:, 768:]).float().cuda())
        else:
            self.data_set = data_utils.TensorDataset(torch.from_numpy(data[:, :768]).float(), torch.from_numpy(data[:, 768:]).float())
        self.data_loader = data_utils.DataLoader(self.data_set, batch_size=self.batch_size, shuffle=True, num_workers=0)
        if validation is not None:
            if torch.cuda.is_available():
                self.validation_set = data_utils.TensorDataset(torch.from_numpy(validation[:, :768]).float().cuda(), torch.from_numpy(validation[:, 768:]).float().cuda())
            else:
                self.validation_set = data_utils.TensorDataset(torch.from_numpy(validation[:, :768]).float(), torch.from_numpy(validation[:, 768:]).float())
    ...
    def predict_move(self, board_image):
        board_image = board_image.reshape(1, 768)
...
def chunk_trainer():
    ...
    try:
        ...
        with open("version_64x12/data", "rb") as f:
    ...
    data_dirs = ["version_64x12/data_puzzle_8"]
